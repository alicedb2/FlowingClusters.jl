module NaiveBIOCLIM
    ...
    function bioclim_predictor(training_predictors::Vector{Vector{Float64}})
        scorerank(x) = 2.0 * (x > 0.5 ? 1.0 - x : x)
        layer_distributions = 1.0 .* eachrow(reduce(hcat, training_predictors))

        function score_new_observation(predictor::Vector{Float64})

            # marginal_scores = [scorerank(quantilerank(training_marginal, predictor_value)) 
            #                     for (training_marginal, predictor_value) in zip(training_marginals, predictor_values)]

            marginal_scores = [scorerank(quantilerank(marginal, val)) for (marginal, val) in zip(layer_distributions, predictor)]

            return minimum(marginal_scores)

        end

        function score_new_observation(predictors::Vector{Vector{Float64}})
            return score_new_observation.(predictors)
        end
    ...
    #     longs, lats = 1.0 .* eachcol(training_observations[:, longlatcols])

    #     return bioclim_predictor(longs, lats, layers)
