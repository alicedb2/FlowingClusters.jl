...
try
    ...
    @warn "The module EvoTrees is not available in the base environment, skipping BRT evaluation"
end
...
if isdefined(Main, :EvoTrees)
    function evaluate_brt(dataset::SMSDataset, species, predictors, perfstat=:MCC; halftraining=false)
        ...
        if !halftraining
            ...
            halftraining = dataset.standardize(predictors...).training.split(1, 1).training
            train_predictors = transpose(halftraining(predictors...))
            train_presabs = halftraining(species)
        end
    ...
    function evaluate_brt(training_predictors, training_presabs, dataset::SMSDataset, species, predictors, perfstat=:MCC)
        ...
        validation_predictors = transpose(validation_dataset.standardize(predictors...)(predictors...))
        validation_presabs = validation_dataset(species)
        gaussian_tree_parameters = EvoTreeGaussian(; loss=:gaussian, metric=:gaussian, nrounds=100, nbins=100, λ=0.0, γ=0.0, η=0.1, max_depth=7, min_weight=1.0, rowsample=0.5, colsample=1.0)
        ...
        test_pres_mask = dataset.test.presmask(species)
        test_abs_mask = dataset.test.absmask(species)
        test_performances = map(x -> round(x, digits=3), performance_statistics(test_predictions[test_pres_mask], test_predictions[test_abs_mask]));
        test_performances_atthresh = map(x -> round(x, digits=3), performance_statistics(test_predictions[test_pres_mask], test_predictions[test_abs_mask], threshold=brt_best_thresh));

        println("#####################")
        println("    BRT stats without thresh: $perfstat=$(getindex(test_performances, perfstat))")
        println("    BRT stats at best thresh: $perfstat=$(getindex(test_performances_atthresh, perfstat))")
