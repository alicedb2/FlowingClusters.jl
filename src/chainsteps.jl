...
function adjust_amwg_logscales!(diagnostics::Diagnostics; acceptance_target=0.44, minmax_logscale=Inf, min_delta=0.01)
    ...
    if di.accepted.pyp.alpha / (di.accepted.pyp.alpha + di.rejected.pyp.alpha) < acceptance_target
        di.amwg.logscales.pyp.alpha -= delta_n
    else
        di.amwg.logscales.pyp.alpha += delta_n
    end

    niw_accrates = di.accepted.niw ./ (di.accepted.niw .+ di.rejected.niw)
    di.amwg.logscales.niw .+= delta_n .* (niw_accrates .< acceptance_target) .- delta_n .* (niw_accrates .>= acceptance_target)

    if !isnothing(di.am)
        nnt_accrates = di.accepted.nn.t ./ (di.accepted.nn.t .+ di.rejected.nn.t)
        di.amwg.logscales.nn.t .+= delta_n .* (nnt_accrates .< acceptance_target) .- delta_n .* (nnt_accrates .>= acceptance_target)
    end
