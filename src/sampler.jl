# function advance_gibbs!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; temperature::T=one(T)) where {T, D}

#     scheduled_elements = shuffle!(elements(clusters))

#     for element in scheduled_elements
#         pop!(clusters, element, delete_empty=false)
...
# function advance_gibbs!(element::Vector{T}, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; temperature::T=one(T)) where {T, D}
...
#         push!(clusters, SetCluster(D, _b2o))
#     end
...
#     if sum(isempty.(clusters)) < 1
#         push!(clusters, C(D, _b2o))
#     end

#     @assert sum(isempty.(clusters)) == 1
#     # filter!(!isempty, clusters)

# end


function advance_alpha!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T))::Int where {T, D}
    return advance_alpha!(default_rng(), clusters, hyperparams, stepsize=stepsize)
end
function advance_alpha!(rng::AbstractRNG, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T))::Int where {T, D}
...
function advance_mu!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::Vector{T}=fill(1/10, D), random_order=true)::Vector{Int} where {T, D}
    return advance_mu!(default_rng(), clusters, hyperparams, stepsize=stepsize, random_order=random_order)
end
function advance_mu!(rng::AbstractRNG, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::Vector{T}=fill(1/10, D), random_order=true)::Vector{Int} where {T, D}
    ...
    for i in dim_order
        ...
        log_acceptance = sum([log_Zniw(c, proposed_mu, lambda, psi, nu) - log_Zniw(EmptyCluster{T, D}(), proposed_mu, lambda, psi, nu) - log_Zniw(c, mu, lambda, psi, nu) + log_Zniw(EmptyCluster{T, D}(), mu, lambda, psi, nu) for c in clusters])
...
function advance_lambda!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T)/10)::Int where {T, D}
        return advance_lambda!(default_rng(), clusters, hyperparams, stepsize=stepsize)
end
function advance_lambda!(rng::AbstractRNG, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T)/10)::Int where {T, D}
    ...
    log_acceptance = sum([log_Zniw(c, mu, proposed_lambda, psi, nu) - log_Zniw(EmptyCluster{T, D}(), mu, proposed_lambda, psi, nu) - log_Zniw(c, mu, lambda, psi, nu) + log_Zniw(EmptyCluster{T, D}(), mu, lambda, psi, nu) for c in clusters])
...
function advance_psi!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D};
                      stepsize::Vector{T}=fill(1/10, div(D * (D + 1), 2)), random_order=true)::Vector{Int} where {T, D}
    return advance_psi!(default_rng(), clusters, hyperparams, stepsize=stepsize, random_order=random_order)
end
function advance_psi!(rng::AbstractRNG, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D};
                      stepsize::Vector{T}=fill(1/10, div(D * (D + 1), 2)), random_order=true)::Vector{Int} where {T, D}
    ...
    for k in dim_order
        ...
        log_acceptance = sum([log_Zniw(cluster, mu, lambda, proposed_psi, nu) - log_Zniw(EmptyCluster{T, D}(), mu, lambda, proposed_psi, nu) - log_Zniw(cluster, mu, lambda, psi, nu) + log_Zniw(EmptyCluster{T, D}(), mu, lambda, psi, nu) for cluster in clusters])
...
function advance_nu!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T))::Int where {T, D}
    return advance_nu!(default_rng(), clusters, hyperparams, stepsize=stepsize)
end
function advance_nu!(rng::AbstractRNG, clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; stepsize::T=one(T))::Int where {T, D}
    ...
    log_acceptance = sum([log_Zniw(c, mu, lambda, psi, proposed_nu) - log_Zniw(EmptyCluster{T, D}(), mu, lambda, psi, proposed_nu) - log_Zniw(c, mu, lambda, psi, nu) + log_Zniw(EmptyCluster{T, D}(), mu, lambda, psi, nu) for c in clusters])
...
# function advance_splitmerge_seq!(clusters::Vector{Cluster}, hyperparams::FCHyperparams; t=3, temperature=1.0)
...
function advance_ffjord!(clusters::AbstractVector{<:AbstractCluster{T, D}}, hyperparams::AbstractFCHyperparams{T, D}; step_distrib=nothing, temperature::T=one(T))::Int where {T, D}
