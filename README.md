...
Of note is the method we use to sample the precision matrix hyperparameter $\Psi$ of the NIW over a flat hyperprior. Since the precision matrix is positive definite, we first use the Cholesky decomposition $\Psi = LL^T$ and perform uniform symmetric moves on the diagonal and lower triangular elements of $L$. To transform those symmetric moves over $L$ into uniform moves over $\Psi$, we use the absolute value of the determinant of the Jacobian
